...
- 默认情况下，根目录 `.env` 中的同名配置优先于进程环境变量；两者都没有时才使用代码默认值
- `CRYPTO_REVERSAL_ORDER_PRICE` 例外：只从根目录 `.env` 读取，不从进程环境变量继承
- 已进入根目录 `.env.example` 的业务关键配置，如果要求显式给定，就不要在代码里再复制一份常量默认值
